...
void CollideLowOrder::Collide_2D_V2(double * &fi,double &rho, double &u, double &v, double *&Fi,double & Fx, double & Fy, double InvTau_tmp, double KineViscosity,double rhor,double rhob){
...
//	std::cout<<rho<<" "<<rhor<<" "<<rhob<<std::endl;
	for(int i=0;i<Nb_VelocityCollide;i++)
	{			
		(this->*PtrCollide_2D_V2)(i,fi[i],rho,u,v,Fi[i],Fx,Fy,InvTau_tmp,rhor,rhob,Phi[0][i],Phi[1][i]);
	}
}
///Collision and add source term on walls
void CollideLowOrder::Collide_2D_V2_Wall(double * &fi,double &rho, double &u, double &v, double *&Fi,double & Fx, double & Fy, double InvTau_tmp, double KineViscosity,double rhor,double rhob){
	//add Hele-Shaw Body Force
	(this->*PtrAddHeleShaw)(u, v,KineViscosity,Fx,Fy,0,0);//Need to update for adding the contribution of the interface forces
	//add user local force
	for(int i=0;i<Nb_VelocityCollide;i++)
	...
	for(int i=0;i<Nb_VelocityCollide;i++)
	{
		(this->*PtrCollide_2D_V2_Wall)(i,fi[i],rho,u,v,Fi[i],Fx,Fy,InvTau_tmp,rhor,rhob,Phi[0][i],Phi[1][i]);
	}
}
void CollideLowOrder::Select_Collide_2D(CollideType type,double Cs2,double referenceDensity,SolverEnum::fluidmodel model){
	...
	else if(model==SolverEnum::DensityRatio){
		PtrEquiDistri=&CollideLowOrder::CompressibleEquiDistriFunct2D;
		std::cerr<< "Compressible equilibrium distribution taken instead of density ratio equilibrium"<<std::endl;
	}
	else
		PtrEquiDistri=&CollideLowOrder::CompressibleEquiDistriFunct2D;
}
...
//	std::cout<<"type: "<<type;
	...
	if(model==SolverEnum::Incompressible)
		PtrEquiDistri=&CollideLowOrder::IncompressibleEquiDistriFunct2D;
	else if(model==SolverEnum::DensityRatio && Param.Get_Model()==SolverEnum::ColourFluid){
		PtrEquiDistri=&CollideLowOrder::DensityRatioEquiDistriFunct2D;
		double alphab=Param.Get_AlphaB();
		double alphar=(Param.Get_DensityRatio()+alphab-1.0)/Param.Get_DensityRatio();
		Phi[0][0]=alphar;Phi[1][0]=alphab;
		for(int i=1;i<5;i++)
			{Phi[0][i]=(1.0-alphar)/5.0;Phi[1][i]=(1.0-alphab)/5.0;}
		for(int i=5;i<9;i++)
			{Phi[0][i]=(1.0-alphar)/20.0;Phi[1][i]=(1.0-alphab)/20.0;}
	}
	else
		PtrEquiDistri=&CollideLowOrder::CompressibleEquiDistriFunct2D;
...
void CollideLowOrder::Collide_2D_SinglePhase(int & i, double &fi,double &rho, double &u, double &v, double & Fx, double & Fy, double & InvTau_tmp){
	fi= fi-InvTau*(fi-CollideEquillibrium(rho, u, v,EiCollide[i], omegaCollide[i],1,1));
}
void CollideLowOrder::Collide_2D_SinglePhase_V2(int & i, double &fi,double &rho, double &u, double &v, double & Fi, double & Fx, double & Fy, double & InvTau_tmp,double &rhor,double &rhob, double &phir, double &phib){
	fi= fi-InvTau*(fi-CollideEquillibrium(rho, u, v,EiCollide[i], omegaCollide[i],rhor,rhob,phir,phib));
}
...
void CollideLowOrder::Collide_2D_SinglePhase_With_LocalForce(int & i, double &fi,double &rho, double &u, double &v, double & Fx, double & Fy, double & InvTau_tmp){
	...
	fi= fi-InvTau*(fi-CollideEquillibrium(rho, u, v,EiCollide[i], omegaCollide[i],1,1))+Fx;
}
void CollideLowOrder::Collide_2D_SinglePhase_With_LocalForce_V2(int & i, double &fi,double &rho, double &u, double &v, double & Fi, double & Fx, double & Fy, double & InvTau_tmp,double &rhor,double &rhob, double &phir, double &phib){
	fi= fi-InvTau*(fi-CollideEquillibrium(rho, u, v,EiCollide[i], omegaCollide[i],rhor,rhob,phir,phib))+Fi;
}
...
//	Fy=Fy+BodyForce(1,rho,u,v);
	fi=fi-InvTau*(fi-CollideEquillibrium(rho, u, v,EiCollide[i], omegaCollide[i],1,1))+Collide_2D_BodyForce(i, u, v, Fx,Fy);
}
void CollideLowOrder::Collide_2D_SinglePhase_With_BodyForce_V2(int & i, double &fi,double &rho, double &u, double &v, double & Fi, double & Fx, double & Fy, double & InvTau_tmp,double &rhor,double &rhob, double &phir, double &phib){
	fi=fi-InvTau*(fi-CollideEquillibrium(rho, u, v,EiCollide[i], omegaCollide[i],rhor,rhob,phir,phib))+Collide_2D_BodyForce(i, u, v, Fx,Fy);
}
///Collision Single phase step with a local (directional) force calculated from User force or model or both
...
void CollideLowOrder::Collide_2D_SinglePhase_With_LocalForceAndBodyForce_V2(int & i, double &fi,double &rho, double &u, double &v, double & Fi, double & Fx, double & Fy, double & InvTau_tmp,double &rhor,double &rhob, double &phir, double &phib){

	fi= fi-InvTau*(fi-CollideEquillibrium(rho, u, v,EiCollide[i], omegaCollide[i],rhor,rhob,phir,phib))+Fi+Collide_2D_BodyForce(i, u, v, Fx,Fy);
}
void CollideLowOrder::No_Collide_2D_SinglePhase_With_LocalForce_V2(int & i, double &fi, double &rho, double &u, double &v, double & Fi, double & Fx, double & Fy, double & InvTau_tmp,double &rhor,double &rhob, double &phir, double &phib){
	fi= fi+Fi;
}
void CollideLowOrder::No_Collide_2D_SinglePhase_With_BodyForce_V2(int & i, double &fi, double &rho, double &u, double &v, double & Fi, double & Fx, double & Fy, double & InvTau_tmp,double &rhor,double &rhob, double &phir, double &phib){
	fi= fi+Collide_2D_BodyForce(i, u, v, Fx,Fy);
}
void CollideLowOrder::No_Collide_2D_SinglePhase_With_LocalForceAndBodyForce_V2(int & i, double &fi, double &rho, double &u, double &v, double & Fi, double & Fx, double & Fy, double & InvTau_tmp,double &rhor,double &rhob, double &phir, double &phib){
...
void CollideLowOrder::Collide_2D_SinglePhase_Non_Constant_Tau(int & i, double &fi,double &rho, double &u, double &v, double & Fx, double & Fy, double & InvTau_tmp){
	fi= fi-InvTau_tmp*(fi-CollideEquillibrium(rho, u, v,EiCollide[i], omegaCollide[i],1,1));
}
void CollideLowOrder::Collide_2D_SinglePhase_Non_Constant_Tau_V2(int & i, double &fi,double &rho, double &u, double &v, double & Fi, double & Fx, double & Fy, double & InvTau_tmp,double &rhor,double &rhob, double &phir, double &phib){
	fi= fi-InvTau_tmp*(fi-CollideEquillibrium(rho, u, v,EiCollide[i], omegaCollide[i],rhor,rhob,phir,phib));
}
...
void CollideLowOrder::Collide_2D_SinglePhase_Non_Constant_Tau_With_LocalForce(int & i, double &fi,double &rho, double &u, double &v, double & Fx, double & Fy, double & InvTau_tmp){
	fi= fi-InvTau_tmp*(fi-CollideEquillibrium(rho, u, v,EiCollide[i], omegaCollide[i],1,1))+Fx;
}
void CollideLowOrder::Collide_2D_SinglePhase_Non_Constant_Tau_With_LocalForce_V2(int & i, double &fi,double &rho, double &u, double &v, double & Fi, double & Fx, double & Fy, double & InvTau_tmp,double &rhor,double &rhob, double &phir, double &phib){
	fi= fi-InvTau_tmp*(fi-CollideEquillibrium(rho, u, v,EiCollide[i], omegaCollide[i],rhor,rhob,phir,phib))+Fi;
}
...
void CollideLowOrder::Collide_2D_SinglePhase_Non_Constant_Tau_With_BodyForce(int & i, double &fi,double &rho, double &u, double &v, double & Fx, double & Fy, double & InvTau_tmp){
	fi=fi-InvTau_tmp*(fi-CollideEquillibrium(rho, u, v,EiCollide[i], omegaCollide[i],1,1))+Collide_2D_BodyForce_Non_Constant_Tau(i, u, v, Fx,Fy, InvTau_tmp);
}
void CollideLowOrder::Collide_2D_SinglePhase_Non_Constant_Tau_With_BodyForce_V2(int & i, double &fi,double &rho, double &u, double &v, double & Fi, double & Fx, double & Fy, double & InvTau_tmp,double &rhor,double &rhob, double &phir, double &phib){
	fi=fi-InvTau_tmp*(fi-CollideEquillibrium(rho, u, v,EiCollide[i], omegaCollide[i],rhor,rhob,phir,phib))+Collide_2D_BodyForce_Non_Constant_Tau(i, u, v, Fx,Fy, InvTau_tmp);
}
///Collision Single phase step with a local (directional) force calculated from User force or model or both and with a local Tau (relaxation time which is related to viscosity and/or Knudsen)
void CollideLowOrder::Collide_2D_SinglePhase_Non_Constant_Tau_With_LocalForceAndUserLocalForce(int & i, double &fi,double &rho, double &u, double &v, double & Fx, double & Fy, double & InvTau_tmp){
	fi= fi-InvTau_tmp*(fi-CollideEquillibrium(rho, u, v,EiCollide[i], omegaCollide[i],1,1))+LocalForce(i, rho, u, v)+Fx;
}
///Collision Single phase step with a body force calculated from User force or model or both and with a local Tau (relaxation time which is related to viscosity and/or Knudsen)
void CollideLowOrder::Collide_2D_SinglePhase_Non_Constant_Tau_With_BodyForceAndUserLocalForce(int & i, double &fi,double &rho, double &u, double &v, double & Fx, double & Fy, double & InvTau_tmp){
	fi=fi-InvTau_tmp*(fi-CollideEquillibrium(rho, u, v,EiCollide[i], omegaCollide[i],1,1))+Collide_2D_BodyForce_Non_Constant_Tau(i, u, v, Fx,Fy, InvTau_tmp)+LocalForce(i, rho, u, v);
}
///Collision Single phase step with a local (directional) force calculated from User force or model or both and with a local Tau (relaxation time which is related to viscosity and/or Knudsen)
void CollideLowOrder::Collide_2D_SinglePhase_Non_Constant_Tau_With_LocalForceAndUserBodyForce(int & i, double &fi,double &rho, double &u, double &v, double & Fx, double & Fy, double & InvTau_tmp){
	...
	fi= fi-InvTau_tmp*(fi-CollideEquillibrium(rho, u, v,EiCollide[i], omegaCollide[i],1,1))+LocalForce(i, rho, u, v)+Collide_2D_BodyForce_Non_Constant_Tau(i, u, v, DVec_2D_tmp[0],DVec_2D_tmp[1], InvTau_tmp);
}
void CollideLowOrder::Collide_2D_SinglePhase_Non_Constant_Tau_With_LocalForceAndBodyForce_V2(int & i, double &fi,double &rho, double &u, double &v, double & Fi, double & Fx, double & Fy, double & InvTau_tmp,double &rhor,double &rhob, double &phir, double &phib){
	fi= fi-InvTau_tmp*(fi-CollideEquillibrium(rho, u, v,EiCollide[i], omegaCollide[i],rhor,rhob,phir,phib))+Fi+Collide_2D_BodyForce_Non_Constant_Tau(i, u, v, Fx,Fy, InvTau_tmp);
}
///Collision Single phase step with a body force calculated from User force or model or both and with a local Tau (relaxation time which is related to viscosity and/or Knudsen)
...
void CollideLowOrder::No_Collide_2D_SinglePhase_Non_Constant_Tau_With_LocalForce_V2(int & i, double &fi, double &rho, double &u, double &v, double & Fi, double & Fx, double & Fy, double & InvTau_tmp,double &rhor,double &rhob, double &phir, double &phib){
	fi=fi+Fi;
}
void CollideLowOrder::No_Collide_2D_SinglePhase_Non_Constant_Tau_With_BodyForce_V2(int & i, double &fi, double &rho, double &u, double &v, double & Fi, double & Fx, double & Fy, double & InvTau_tmp,double &rhor,double &rhob, double &phir, double &phib){
	fi=fi+Collide_2D_BodyForce_Non_Constant_Tau(i, u, v, Fx,Fy, InvTau_tmp);
}
void CollideLowOrder::No_Collide_2D_SinglePhase_Non_Constant_Tau_With_LocalForceAndBodyForce_V2(int & i, double &fi, double &rho, double &u, double &v, double & Fi, double & Fx, double & Fy, double & InvTau_tmp,double &rhor,double &rhob, double &phir, double &phib){
