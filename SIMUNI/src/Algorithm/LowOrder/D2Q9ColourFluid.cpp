...
//Initialise boundary conditions.
	InitD2Q9Bc(Dic, NodeArrays,Parameters__,Ei,2);
//Set_Convergence
	Set_Convergence();
...
// Select Pressure calculation
	if(PtrParameters->Get_ModelOfFluid()==SolverEnum::DensityRatio)
		PtrPressure=&D2Q9ColourFluid::IdealGazIsothermalDensityRatioPressure;
	else
		PtrPressure=&D2Q9ColourFluid::IdealGazIsothermalPressure;	
...
void D2Q9ColourFluid::Set_Recolouring(){
	...
	if(PtrParameters->Get_ModelOfFluid()==SolverEnum::DensityRatio)
		PtrRecolour=&D2Q9ColourFluid::Recolouring_LatvaDensityRatio;	
...
//Initialise parameters
	...
	alphab=PtrParameters->Get_AlphaB();
	alphar=(PtrParameters->Get_DensityRatio()+alphab-1.0)/PtrParameters->Get_DensityRatio();
...
// Initialise the blue and red densities
		...
		for (int i=0;i<nbvelo;i++)
		{
			f[0]->f[i][idx]=CollideLowOrder::CollideEquillibrium(Rhor[idx], U[0][idx], U[1][idx], &Ei[i][0], omega[i],Rhor[idx],0,Phi[0][i],Phi[1][i]);
			f[1]->f[i][idx]=CollideLowOrder::CollideEquillibrium(Rhob[idx], U[0][idx], U[1][idx], &Ei[i][0], omega[i],0,Rhob[idx],Phi[0][i],Phi[1][i]);
		}
	}
	for (unsigned int j=0;j<NodeArrays->NodeSpecialWall.size();j++)
	{
...
void D2Q9ColourFluid::Recolouring_LatvaDensityRatio(int & nodenumber, double * fi_tmp){
	if(G_Norm[nodenumber]<LimitGNorm)
	{
		if(Rhor[nodenumber]<Rhob[nodenumber])
			for(int i=0;i<nbvelo;i++)
	...
	else
	{
		...
		double Rhor_Rho=Rhor[nodenumber]/Rho[nodenumber];
		double Rhob_Rho=Rhob[nodenumber]/Rho[nodenumber];
		double factor=beta*Rhor[nodenumber]*Rhob[nodenumber]/(Rho[nodenumber]*Rho[nodenumber]);
		...
		{
				f[0]->f[i][nodenumber]=Rhor_Rho*fi_tmp[i]
							+factor*omega[i]*CosPhi(nodenumber,i,G_Norm[nodenumber])*(Rhor[nodenumber]*Phi[0][i]+Rhob[nodenumber]*Phi[1][i]);
				f[1]->f[i][nodenumber]=fi_tmp[i]-f[0]->f[i][nodenumber];//Rhob_Rho*fi_tmp[i]
		}
	}
}
...
//		Collide_2D(i, fi[i],Rho[nodenumber], U[0][nodenumber], U[1][nodenumber], Collision_operator_Grunau(i, nodenumber, A1),DVec_2D_tmp[1], Get_InvTau(Rho[nodenumber],RhoN[nodenumber]));
	...
	Collide_2D_V2(fi,Rho[nodenumber], U[0][nodenumber], U[1][nodenumber], Fi, Fx,Fy, Get_InvTau(Rho[nodenumber],RhoN[nodenumber]),Get_Mu(Rho[nodenumber],RhoN[nodenumber]),Rhor[nodenumber],Rhob[nodenumber]);
}
...
//		Collide_2D(i, fi[i],Rho[nodenumber], U[0][nodenumber], U[1][nodenumber], Collision_operator_Reis(i, nodenumber, Ak),DVec_2D_tmp[1], invtau);
	...
	Collide_2D_V2(fi,Rho[nodenumber], U[0][nodenumber], U[1][nodenumber], Fi, Fx,Fy, invtau,Get_Mu(Rho[nodenumber],RhoN[nodenumber]),Rhor[nodenumber],Rhob[nodenumber]);
}
...
//		Collide_2D(i, fi[i],Rho[nodenumber], U[0][nodenumber], U[1][nodenumber], F[0][nodenumber],F[1][nodenumber], Get_InvTau(Rho[nodenumber],RhoN[nodenumber]));
	...
	Collide_2D_V2(fi,Rho[nodenumber], U[0][nodenumber], U[1][nodenumber], Fi, F[0][nodenumber],F[1][nodenumber], Get_InvTau(Rho[nodenumber],RhoN[nodenumber]),Get_Mu(Rho[nodenumber],RhoN[nodenumber]),Rhor[nodenumber],Rhob[nodenumber]);
}
...
*/
	...
	{
//Old version save
		ApplyWall(NodeArrays->NodeWall[j],NodeArrays->NodeWall[j].Get_BcNormal(),NodeArrays->NodeWall[j].Get_connect(),f[0],Rhor,U[0],U[1],0);
		ApplyWall(NodeArrays->NodeWall[j],NodeArrays->NodeWall[j].Get_BcNormal(),NodeArrays->NodeWall[j].Get_connect(),f[1],Rhob,U[0],U[1],1);
...
//	std::cout<<"Apply bc "<<"f6: "<<f[1]->f[6][idxcheck]<<" f8: "<<f[1]->f[8][idxcheck];
	...
	for (unsigned int j=0;j<NodeArrays->NodeCorner.size();j++)
	{
		ApplyCornerWall(NodeArrays->NodeCorner[j], f[0],Rhor,U[0],U[1],0);
		ApplyCornerWall(NodeArrays->NodeCorner[j], f[1],Rhob,U[0],U[1],1);
	}
/*
...
void D2Q9ColourFluid::CalculatePressure(int const &idx){
	P[idx]=(this->*PtrPressure)(Rho[idx],Rhor[idx],Rhob[idx]);
	//P[idx]=IdealGazIsothermalPressure(Rho[idx]);
}
double D2Q9ColourFluid::IdealGazIsothermalPressure(double const &Rho,double const &Rhor,double const &Rhob){
	return Rho/3.0;
}

double D2Q9ColourFluid::IdealGazIsothermalDensityRatioPressure(double const &Rho,double const &Rhor,double const &Rhob){
	return (Rhor*(1.0-alphar)+Rhob*(1.0-alphab))*3.0/5.0;
}
