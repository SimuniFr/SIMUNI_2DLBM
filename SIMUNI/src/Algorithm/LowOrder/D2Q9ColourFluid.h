...
// Macroscopic calculation
	...
	double IdealGazIsothermalPressure(double const &Rho,double const &Rhor,double const &Rhob);
	double IdealGazIsothermalDensityRatioPressure(double const &Rho,double const &Rhor,double const &Rhob);
...
/*
	...
	*/
	...
	void UpdateColourGradientOnWalls();
	void UpdateColourGradient(int* connect, int & normal);
...
// Recolouring definition
	...
	void Recolouring_LatvaDensityRatio(int & nodenumber, double * fi_tmp);
...
//	void CollideD2Q9ColourFluid(int & direction, double & fi,double &rho,double*  F,double & F_Norm, double & InvTau_, double &u, double &v);
	void Set_Collide();
	void Select_Colour_Operator(ColourFluidEnum::ColourOperatorType OperatorType_);
	...
	void SurfaceForceBc(int & nodenumber, int* connect,int & normal,double & Fx,double & Fy);
	void SurfaceForceCorner(int & nodenumber, int* connect,int & normal,double & Fx,double & Fy);
	...
	void Collision_SurfaceForceBc(int & nodenumber, int* connect,int & normal,double* fi);
	void Collision_SurfaceForceCorner(int & nodenumber, int* connect,int & normal,double* fi);

	double Curvature(int & nodenumber, int* connect,int & normal);
	double CurvatureBc(int & nodenumber, int* connect,int & normal);
	double CurvatureCorner(int & nodenumber, int* connect,int & normal);
	...
	void Set_Extrapolations();
	void ExtrapolDensityInSolid();
...
//Multiphase variables
	...
	double alphar,alphab;///< Parameters for density ratio
...
///Simplify notation for pointer on a member function of D2Q9ColourFluid class for the calculation of pressure
	typedef double(D2Q9ColourFluid::*Pressure)(double const &Rho,double const &Rhor,double const &Rhob);
